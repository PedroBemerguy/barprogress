...
def bar(loader,bar_progress_l,bar_empty_l,g_l,time_total):
...
def loader(bar_progress_p,bar_empty_p,g_p,time_trans):
    # dict = {
    #   1: bar('-',bar_progress_p,bar_empty_p,g_p),
    #   2: bar('/',bar_progress_p,bar_empty_p,g_p),
    #   3: bar('|',bar_progress_p,bar_empty_p,g_p),
    #   4: bar('\\',bar_progress_p,bar_empty_p,g_p)
    # }
...
if (i == -1):
    try:
      ...
      for g in gain_list:
            ...
            p = multiprocessing.Process(target=loader,args=(bar_progress,bar_empty,g,time.time()-time_init,))
            p.start()
            time.sleep(1.2)
            p.terminate()
