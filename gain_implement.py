...
class bar_class():
    ...
    def loader(self,bar_progress_p,bar_empty_p,g_p,time_trans):
          ...
          while True:
            for i in range(1,5):
                dict = {
                      1: self.bar('-',bar_progress_p,bar_empty_p,g_p,time_trans+time.time()-time_init_loader),
                      2: self.bar('/',bar_progress_p,bar_empty_p,g_p,time_trans+time.time()-time_init_loader),
                      3: self.bar('|',bar_progress_p,bar_empty_p,g_p,time_trans+time.time()-time_init_loader),
                      4: self.bar('\\',bar_progress_p,bar_empty_p,g_p,time_trans+time.time()-time_init_loader)
                }
